// From https://github.com/ReactiveCocoa/ReactiveCocoa/blob/a6d5dfc56c760109e57b10551925974fdfc1c6e0/ReactiveCocoa/NSObject%2BAssociation.swift
...
extension NSObjectProtocol {
    ...
    internal func associatedValue<T>(forKey key: StaticString = #function, initial: (Self) -> T) -> T {
        let key = AssociationKey<T?>(key)

        if let value = self.associations.value(forKey: key) {
            return value
        }

        let value = initial(self)
        self.associations.setValue(value, forKey: key)
